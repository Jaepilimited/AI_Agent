...
Qdrant Cloud 색인에서 `last_edited_time` 이 비어 있는 **notion 소스** 문서다.

> 현재(2026-08-25 11:23 재측정): notion 소스 460 청크 중 **46 청크 · 22 페이지**.
> 처음 적을 때는 색인이 1,854 청크였는데, 같은 날 낡은 사본 568개를 정리하고
> 크롤 깊이를 늘려 1,455가 됐다 — 분모가 달라졌을 뿐 이 목록의 페이지는 그대로다.
...
```
python scripts/check_notion_sharing.py            # 이 목록으로 확인
python scripts/check_notion_sharing.py --rescan   # 색인을 다시 훑어 목록 갱신 후 확인
```

⚠️ **공유 여부는 파이프라인을 기다리지 않고 지금 알 수 있다** — 공유 안 된 페이지는
`GET /v1/pages/{id}` 가 404 를 준다. 색인 반영은 그 다음 문제다.
`--rescan` 이 0개를 내면 끝난 것이다.

**2026-08-25 확인: 17개 전부 미공유 (공유됨 0).**
URL 이 기록되지 않은 4개(`Craver`·`DB`·`IT`·`임베딩 된 구글시트`)는 확인 대상에서 빠진다.
