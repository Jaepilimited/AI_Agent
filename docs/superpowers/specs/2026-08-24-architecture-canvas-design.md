...
> ⛔ **이 다이어그램은 처음에 틀려 있었고, 네 개 태스크가 그걸 충실히 구현했다.**
> 아래는 2026-08-24 최종 리뷰에서 코드를 다시 읽어 고친 판이다. 무엇이 어떻게
> 틀렸었는지는 이 절 끝의 "다이어그램이 틀렸던 기록"에 남긴다 — 지우면 다음 사람이
> 같은 흐름을 다시 유도해 낸다.

```
USER INPUT
  → @@ 소스 파싱
  → 초상권 가로채기 ──(@@초상권 · 초상권 의도)──▶ model_rights
  → 보고서   가로채기 ──(@@보고서 · wants_report)──▶ report
  → 소스 지정 경로   ──(@@ 지정 · 사이드바 단일 소스)──▶ bigquery · notion · cs · gws
  → 후속경로 상속    ──(직전 경로 · 정정 후속)──▶ bigquery · notion · gws · cs
  → 라우터: 키워드 (_keyword_classify_ex)
       ├─ 확신   ─────────────┐
       └─ 확신없음 → 라우터: LLM ┤
                              ↓
        bigquery · notion · gws · cs · multi · direct     ← 여섯. 이게 전부다
                              ↓
                           응답

  bigquery 만: route.bigquery → [LangGraph] → format_answer → 답변 수치검증 → 응답
  team: 도달 불가 (핸들러는 살아 있으나 어떤 진입점도 만들지 않는다)
```

**분류기가 낼 수 있는 값은 여섯이다** — `_keyword_classify_ex` 의 모든 `return`
리터럴과 `_classify_with_llm` 의 허용 집합이 정확히 일치한다.
`orchestrator.ROUTER_ROUTES` 가 그 단일 소스이고, 정적 검사가 함수 본문에서 직접
읽어 대조한다.

**`report`·`model_rights` 는 라우트가 아니라 관문이다.** 분류기가 돌기도 **전에**
가로채고 그 자리에서 return 한다 (스트리밍·비스트리밍 양쪽 모두). 그래서 캔버스에서
라우터보다 **위**에 있다 — CLAUDE.md 가 이 화면에 요구하는 *"왜 보고서가 안
만들어지지"* 의 답이 여기 있기 때문이다. 하류로 그리면 admin 이 라우터를 뒤지게 되고,
거기엔 아무것도 없다.

**`answer_check` 는 bigquery 전용이다.** 호출부가 앱 전체에 하나뿐이다
(`sql_agent.format_answer` 안). `multi` 도 `_multi_prepare` 로 sql_agent 를 부르지만
그 답은 Flash 합성을 한 번 더 거쳐 나가므로 **최종 답변이 검증된 것이 아니다** —
그리지 않는다.

#### 다이어그램이 틀렸던 기록 (2026-08-24)

최종 리뷰가 그린 엣지 37개 중 **13개가 코드가 하지 않는 일을 주장**하고 있었다.
전부 이 문서의 옛 ASCII 그림에서 그대로 내려온 것이다.

| 무엇 | 몇 개 | 왜 위험했나 |
|---|---|---|
| 라우터 → `report`·`model_rights`·`team` | 6 | 분류기는 여섯만 낸다. `report`·`model_rights` 는 **방향까지 거꾸로**였다 (관문이 라우터보다 위) |
| 모든 경로 → `answer_check` | 7 | 수치검증이 덮지 않는 경로를 덮는다고 주장 — 이 기능이 낼 수 있는 최악의 사고 |

- ⛔ **"실측 확인함"의 근거가 낱말 출현 빈도였다.** 옛 문서에는 *"라우트 9종은 실측
  확인함 (출현 빈도: bigquery 57 · … · report 4)"* 라고 적혀 있었다. grep 횟수는
  **그 이름이 파일에 몇 번 나오는지**일 뿐 제어 흐름이 아니다. `report` 가 4번
  나온다는 사실은 그게 라우터의 출력이라는 근거가 전혀 못 된다. 흐름을 주장하려면
  **반환 리터럴과 호출 순서**를 읽어야 한다.
- ⛔ **캔버스가 자기 명제를 어겼다.** *그래프는 생성된다, 그려지지 않는다* 인데,
  정작 `spec.py` 가 이 손그림을 옮겨 적었다. 선언이 손으로 유지되는 한 "선언이
  코드와 같은가"를 보는 검사가 **선언의 모든 축**을 덮어야 한다.
- **정적 검사도 이 13개를 못 잡았다.** 역방향 검사가 `_DB_REGISTRY` 만 봐서
  라우터 전용 경로(`direct`·`team`·`multi`)가 아예 안 보였고, **엣지는 전혀
  검사하지 않았다** (노드 존재만 봤다). 지금은 세 출처의 합집합 ⊆ 노드 + 라우터
  나가는 엣지 == `ROUTER_ROUTES` 를 함께 본다.
- 💡 **불완전한 그림보다 틀린 그림이 나쁘다.** 빠진 그림은 사람이 코드를 더 읽게
  만들지만, 틀린 그림은 코드를 **안 읽게** 만든다. 확인 못 한 경로는 그리지 않는다
  (현재 미반영: FI 권한 차단 조기 return, 성분 질의 가로채기, 이미지 첨부 시 direct
  강제 — 전부 관문류이고, 그릴 때 코드로 확인하고 넣을 것).
