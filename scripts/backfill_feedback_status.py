...
ENTRIES = [
    (144, "2026-08-24", "같은 질문인데 기간의 끝이 매번 달라져 총량이 갈리던 것을 "
                        "고쳤습니다. '8월' 처럼 달을 지목하면 이제 항상 그 달 전체를 "
                        "조회합니다 (8/1~8/31)."),
    (143, "2026-08-24", "같은 질문인데 기간의 끝이 매번 달라져 총량이 갈리던 것을 "
                        "고쳤습니다. 1,805개로 나온 조회는 8/24 까지만 세고 있었고, "
                        "그 사실을 답변에 적지 않았습니다. 이제 '8월' 은 항상 8/1~8/31 입니다."),
    (138, "2026-08-19", "차트 툴팁에 '제품명: 숫자' 로 뜨던 것을 고쳤습니다. 시리즈 "
                        "이름 자리에 x축 이름이 들어가 있었습니다 — 이제 그 자리에는 "
                        "측정값(총 매출 등)만 들어갑니다."),
    (126, "2026-08-06", "SQL 오류(PARTITION BY … neither grouped nor aggregated)가 나면 "
                        "자동으로 다시 만들도록 8/14 에 고쳤습니다. 지금은 같은 질문이 "
                        "정상 조회됩니다."),
    (120, "2026-08-05", "'유럽' 조회가 0건이던 것을 고쳤습니다. 원인은 '유럽' 이 없는 "
                        "세부 권역 컬럼에 조건이 걸렸던 것입니다 — 광역 대륙 컬럼에는 "
                        "'유럽' 이 있습니다. 이제 값이 어느 컬럼에 있는지 보고 자동으로 "
                        "바로잡습니다."),
    # ── 차트 (임재필, 사흘간 5건) ────────────────────────────────────────────
    (147, "2026-08-25", "BigQuery 테이블 경로가 답변 본문에 나가던 것을 막았습니다. "
                        "이제 코드가 가립니다. 다만 '경로 = 원본 URL' 로 답하는 기능은 "
                        "아직 없습니다 — 소스별 원본 링크를 등록해야 합니다."),
