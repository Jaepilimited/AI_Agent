...
# ⛔ `_rate()` 는 이 목록을 **순서대로** 훑어 첫 부분문자열 일치를 쓴다 — 길이로
#   정렬하지 않는다. 좋은 자리에 일반항을 두면 구체항이 영영 안 걸린다 —
#   `flash-lite` 를 `flash` 보다 **앞에** 둔 이유가 그것이다.
# ⛔ 2026-09-07 실측으로 거의 전부를 고쳤다. flash 행이 **Flash-Lite 요율**(0.30/2.50)
#   이었고, 그것이 그대로 쓰이면서 비용 대시보드가 Gemini 를 입력 5배·출력 3.6배
#   **낮게** 적고 있었다. 에러가 아니라 그럴듯한 숫자라 아무도 못 알아챈다.
# ⚠️ Gemini 3.x Flash 는 2026-12-31 까지 프로모션 요율이다 — 2027-01-01 부터 **2배**
#   (0.75/3.75 → 1.50/7.50). 그때 이 표를 함께 고칠 것.
# ⚠️ 3.1 Pro 는 200k 토큰 초과 프롬프트가 4.00/18.00 으로 올라간다 — 여기엔
#   일반 구간만 적는다 (그만큼 과소추정이다).
_PRICING = [
    # (모델명 조각, 입력 $/1M, 출력 $/1M)
    ("opus", 15.0, 75.0),
    ("sonnet", 3.0, 15.0),
    ("haiku", 0.8, 4.0),
    ("fable", 15.0, 75.0),
    # Gemini 3.x Flash 계열 (2026-12-31 까지 프로모션)
