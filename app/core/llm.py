...
#: 시도와 시도 **사이**의 대기 (초). ⛔ 마지막 시도 뒤에는 자지 않는다 — 기다려 봐야
#: 다음 시도가 없다. 그래서 **대기 횟수는 시도 횟수보다 하나 적다.**
_RETRY_DELAYS = [0.5, 1.5, 4.0]

#: 총 시도 횟수. ⛔ 손으로 적지 말고 대기 목록에서 파생시킨다 — 따로 적으면 어긋나고,
#: 실제로 어긋나 있었다 (2026-08-27 실측). `[0.5, 1.5, 4.0, 10.0]` 에 `_MAX_RETRIES = 4`
#: 였는데 **마지막 10.0초는 한 번도 쓰이지 않았다**: 4번째 시도는 실패하면 대기 없이
#: 바로 올라오기 때문이다. 정의를 읽으면 16초를 기다릴 것 같은데 실제 대기는 6초였다.
#: 에러가 아니라 **문서와 동작의 불일치**라 재보기 전까지 아무도 몰랐다.
