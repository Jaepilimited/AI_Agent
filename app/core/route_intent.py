...
def source_free_footer() -> str:
    ...
    ⛔ **머리말을 바꾸지 마라 — 이 문구가 칩이 되는 유일한 조건이다.**
       `chat.js.extractFollowupsFromAnswer()` 는 💡 줄이
       `/물어보세요|질문|follow[- ]?up|try asking|ask these/i` 에 걸릴 때만
       후속 칩으로 만든다. 처음 문안("사내 데이터로 확인해 드릴까요?")은 어디에도
       안 걸려 **답변 맨 끝의 평문**으로 렌더됐다 — 이 저장소가 두 번 "거기는
       아무도 안 읽는다" 고 결론 낸 자리다 (물류 이상치 공시·OP 재고 신선도).
