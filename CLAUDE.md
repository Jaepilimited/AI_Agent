...
- ⛔ **direct 경로 시스템 프롬프트는 두 벌이다** — `_build_direct_system_prompt()`(스트리밍)과
  `_handle_direct()` 안의 인라인 사본(비스트리밍). **한쪽만 고치면 경로에 따라 답이 갈린다.**
  실제로 보고서 기능 설명을 한쪽에만 넣었더니 *"보고서라는 별도 메뉴는 없습니다"* 라고
  답했다 (2026-08-13). `tests/test_report_rules.py` 가 두 사본의 어긋남을 잡는다
- ⛔ **데이터 낱말은 세 목록에 함께 넣어야 한다** (2026-08-13 실측). `_DATA_KEYWORDS` 만
  고치면 뒤의 두 관문에서 걸려 **direct 로 떨어진다** — 웹검색 가드의 `_SKIN1004_TERMS`,
  마지막 `_BIZ_CONTEXT`. 실제로 "할인 많이 준 채널 어디야"·"신제품 몇 개나 나왔어"·
  "FOC 무상출고 비용 얼마나 나갔어" 가 전부 direct 로 새고 있었다
