...
- **브랜드 = 스킨천사(SK) · 커먼랩스(CL) · 좀비뷰티 3개뿐** (2026-08-05 확정)
  - ⛔ **`Brand` 컬럼 값을 브랜드로 믿지 마라.** 실측(2025~) 결과 `Brand` 에는 SK·UM·CBT·CL 이 있는데
    **CBT 는 팀**이다 — `Team_NEW` 의 CBT 와 건수·금액이 완전히 일치한다 (1,919,601건 / 659.0억)
  - **좀비뷰티는 `Brand` 에 없다** — `Brand='SK' AND Line='ZB'` 로 들어 있다 (63,923건 / 18.7억)
  - **CBT·JBT·KBT·EAST·WEST·DT·B2B 는 팀** → `Team_NEW` 로 답할 것. 브랜드로 나열 금지
  - "브랜드별" 질문에 `GROUP BY Brand` 를 그대로 쓰면 팀이 브랜드로 섞여 나온다
  - ⚠️ `UM`(3,251.7억) 과 `Product` 테이블의 `DD` 는 정체 미확인 — 브랜드로 단정하지 말 것
