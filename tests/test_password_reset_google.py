...
# ─────────────── 되지 않는 경로의 입구는 두지 않는다 ───────────────
# ⛔ 구글 정책: 리다이렉트는 https 여야 하고 예외는 localhost 뿐이다.
#    우리 콜백이 http 라 계정을 고른 뒤 차단된다 — 기존 GWS 연결도 같이 막힌다.

@pytest.mark.parametrize("uri,expected", [
    ("https://cella.example.com/auth/google/callback", True),
    ("http://localhost:3000/auth/google/callback", True),
    ("http://127.0.0.1:3000/auth/google/callback", True),
    ("http://10.1.100.5.nip.io/auth/google/callback", False),   # 지금 프로덕션
    ("http://cella.example.com/auth/google/callback", False),
])
def test_google_path_is_offered_only_when_it_can_actually_work(monkeypatch, uri, expected):
    class _S:
        google_oauth_client_id = "cid"
        google_oauth_client_secret = "s"
    monkeypatch.setattr(pwg, "get_settings", lambda: _S())
    assert pwg.is_available(uri) is expected


def test_the_server_blocks_the_dead_end_too_not_just_the_screen():
    """화면에서 숨기는 것만으로는 부족하다 — 주소를 직접 치면 구글 차단 화면을 본다."""
    from app.api import auth_api

    src = inspect.getsource(auth_api.password_reset_google_start)
    assert "is_available" in src
    assert "503" in src


def test_the_screen_asks_the_server_which_paths_work():
    """⚠️ 프론트에 조건을 박으면 HTTPS 를 켜는 날 아무도 기억하지 못한다."""
    import io as _io
    import os as _os

    root = _os.path.dirname(_os.path.dirname(_os.path.abspath(__file__)))
    js = _io.open(_os.path.join(root, "app", "frontend", "auth.js"), encoding="utf-8").read()
    assert "/api/auth/password-reset/options" in js
    assert "https" not in js.split("hideDeadEnds")[1][:600], "프론트가 스스로 판정한다"
