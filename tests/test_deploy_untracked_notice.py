...
def test_a_modified_tracked_file_is_not_flagged(tmp_path):
    """⚠️ 고쳐 둔 것(`M`)은 알리지 않는다 — 이 트리는 세션 서넛이 공유해서
    `M` 이 상시 100개를 넘는다. 그것까지 세면 경고가 매번 떠서 죽는다.

    ⛔ **"`M` 은 잃을 수 없다" 고 읽지 마라.** 잃을 수 없는 것은 *파일*이고,
       거기 담긴 **미커밋 변경 자체는 잃는다** — 트리가 사라지면 그 파일은
       HEAD 상태로 돌아간다. 추적 안 된 것과의 차이는 *"어디에도 없다"* 와
       *"직전 상태로 돌아간다"* 이고, 그건 **심각도 차이지 무손실이 아니다.**
       (2026-09-09 ai-agent-de 세션이 이 문구를 바로잡아 줬다. 이 주석을
       근거로 다음 사람이 안심하면 안 된다.)

    그래도 알리지 않는 쪽을 고른 것은 **경고의 수명** 때문이다. 넓히면 매번
    떠서 아무도 안 읽고, 그러면 진짜 신호(추적 안 된 로그인 경로 코드)까지
    함께 묻힌다.
    """
    root = _repo(tmp_path)
