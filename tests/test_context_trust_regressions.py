...
def test_missing_date_cap_never_kills_the_request():
    ...
    ⚠️ 게다가 그 관문은 판정부터 깨져 있었다. 상한 정규식이 따옴표 뒤에 날짜만 오는
       ...
       프로덕션 14일 실측: 발동 10회 중 **9회가 오탐**이었다 — 거부당한 SQL 에 상한이
       실제로 들어 있었다. 그 탓에 요청 5건이 죽었고, 한 사용자는 같은 질문을 5분간
       5번 던져 5번 다 "SQL이 생성되지 않았습니다" 를 받았다.
       ⚠️ "성공 몇 회" 는 셀 수 없다 — 성공 로그가 INFO 라 프로덕션에서 버려진다.
    """
