...
@pytest.mark.parametrize("question", [
    ...
    "우마 브랜드 매출 리포트 뽑아줘",
    # 업무 문서 낱말이 섞여도 **분석 데이터**를 명시해 만들라고 하면 보고서다
    "2026년 B2B2 매출 보고서 만들어줘",
])
...
@pytest.mark.parametrize("question", [
    ...
    "보고서 만드는 방법 알려줘",
    # 2026-08-19 실측: '출장 보고서'라는 업무 문서를 매출 분석 산출물로 오인해
    # 질문과 무관한 2026 상반기 B2B 매출 보고서를 7개 절로 생성했다.
    "출장 보고서와 정산서 작성 시 B2B2팀의 내부 역할 분담 규정은 어떻게 되나요?",
    "출장 보고서와 일정표는 별도로 작성해야 하나요?",
    "정산 보고서 양식은 어디서 다운로드받을 수 있나요?",
    # 업무 문서를 작성해 달라는 요청도 지원하지 않는 매출 분석 보고서로 만들지 않는다
    "출장 보고서 작성해줘",
    # 신호어가 아예 없다
...
@pytest.mark.parametrize("question,expected", [
    ...
    ("온라인 판매 가이드라인", "notion"),
    # '보고서'는 분석 산출물 이름이 아니라 사내 업무 문서다. 보고서 생성 앞단에서
    # 막힌 뒤 이 결정론적 Notion 라우트로 와야 한다.
    ("출장 보고서와 정산서 작성 시 B2B2팀의 내부 역할 분담 규정은 어떻게 되나요?", "notion"),
